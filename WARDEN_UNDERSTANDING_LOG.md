...
### Loading a Non-Package Script as a Module with `importlib.util`
**Part of:** Phase 3 integration — `demo/run_phase3_demo.py`
**Why it came up:** `demo/run_phase3_demo.py` needed to import `seed` and constants from `demo/seed_phase3_data.py`, but `demo/` has no `__init__.py` — it is a scripts directory, not a Python package. A normal `from demo.seed_phase3_data import ...` would raise `ModuleNotFoundError`.

**Plain English:**
Python normally finds code you want to reuse by looking in folders that have been declared as "packages" (using a special `__init__.py` file). The `demo/` folder is just a collection of standalone scripts, not a package, so normal imports don't work between them. Python's `importlib.util` is a lower-level tool that lets you load any `.py` file from its actual location on disk, bypassing the package requirement entirely. You give it the file path, and it returns the module ready to use, exactly as if you had imported it normally.

**Technical:**
`importlib.util.spec_from_file_location(name, path)` constructs a `ModuleSpec` that describes the module's source file without requiring it to be on `sys.path` or inside a package. `importlib.util.module_from_spec(spec)` creates the module object, and `spec.loader.exec_module(module)` executes the file's top-level code into that object. After these three lines, `module.seed`, `module.SEED_SCENARIOS`, etc. are accessible. The `name` argument is arbitrary — it sets `__name__` inside the loaded module but has no effect on how Python resolves other imports within it (those still use the normal `sys.path`).

```python
import importlib.util
_spec = importlib.util.spec_from_file_location("seed_phase3_data", Path(__file__).parent / "seed_phase3_data.py")
_mod  = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_mod)
seed = _mod.seed
```

This pattern is appropriate for script directories (like `demo/`) where adding `__init__.py` would be misleading, and for loading plugins or configuration files from known paths at runtime.

---

### Synthetic Seed Data as an Integration Test Strategy
**Part of:** Phase 3 integration — `demo/seed_phase3_data.py` + `tests/test_seed_phase3_data.py`
**Why it came up:** The Phase 3 scanner requires a ledger with enough FLAG events to cross N=10 within the 6h window. Real agent sessions don't reliably produce this volume in a one-shot demo or test run.

**Plain English:**
Rather than running a real agent for many hours to produce enough flagged network events, we insert fake-but-realistic rows directly into the database before the demo or test runs. These rows look exactly like what the live daemon would write — same columns, same JSON format, same verdict values — but they are generated by a script instead of real agent activity. We include two kinds of fake data: "positive controls" (events that should trigger the scanner) and "negative controls" (events designed to stay silent, proving the scanner respects both the event count threshold and the rolling time window). Running both through the scanner proves the system works correctly without needing a multi-hour live agent session.
