...
This is explicitly advisory, not enforcement. It's the one place an LLM appears in the system, and it's kept strictly downstream of the deterministic engine so Phase 1/2's guarantees ("every decision is deterministic and reproducible") remain true for the enforcement path itself.

*(Deferred in full detail until Phase 2 is solid.)*
